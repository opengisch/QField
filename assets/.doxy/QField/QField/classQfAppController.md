...
[**ClassList**](annotated.md) **>** [**QfAppController**](classQfAppController.md)



_Application operations that_ [_**QfAppInterface**_](classQfAppInterface.md) _delegates to the app._
...
| virtual  | [**~QfAppController**](#function-qfappcontroller) () = default<br> |
...
```C++
virtual void QfAppController::clearProject () = 0
```
...
```C++
virtual bool QfAppController::loadProjectFile (
    const QString & path,
...
```C++
virtual bool QfAppController::print (
...
```C++
virtual bool QfAppController::printAtlasFeatures (
    const QString & layoutName,
...
```C++
virtual bool QfAppController::readProjectBoolEntry (
    const QString & scope,
    const QString & key,
    bool def=false
) const = 0
...
```C++
virtual double QfAppController::readProjectDoubleEntry (
    const QString & scope,
    const QString & key,
    double def=0.0
) const = 0
...
```C++
virtual QString QfAppController::readProjectEntry (
    const QString & scope,
    const QString & key,
...
```C++
virtual void QfAppController::readProjectFile () = 0
```
...
```C++
virtual int QfAppController::readProjectNumEntry (
    const QString & scope,
    const QString & key,
    int def=0
) const = 0
...
```C++
virtual void QfAppController::reloadProjectFile () = 0
```
...
```C++
virtual void QfAppController::setScreenDimmerTimeout (
    int timeoutSeconds
) = 0
```
...
### function ~QfAppController 

```C++
virtual QfAppController::~QfAppController () = default
```
...
------------------------------
The documentation for this class was generated from the following file `src/core/qfappcontroller.h`
