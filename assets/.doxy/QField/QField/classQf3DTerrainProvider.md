

# Class Qf3DTerrainProvider



[**ClassList**](annotated.md) **>** [**Qf3DTerrainProvider**](classQf3DTerrainProvider.md)



[More...](#detailed-description)

* `#include <qf3dterrainprovider.h>`



Inherits the following classes: QObject


























## Public Properties

| Type | Name |
| ---: | :--- |
| property QgsRectangle | [**extent**](classQf3DTerrainProvider.md#property-extent-12)  <br>_Geographic extent used for terrain data._  |
| property bool | [**forceSquareSize**](classQf3DTerrainProvider.md#property-forcesquaresize-12)  <br>_Whether the extent's shorter side will be expanded to form a square terrain._  |
| property QSize | [**gridSize**](classQf3DTerrainProvider.md#property-gridsize-12)  <br>_Grid dimensions (width x height) for terrain sampling._  |
| property bool | [**isLoading**](classQf3DTerrainProvider.md#property-isloading-12)  <br>_Whether terrain data is currently being loaded._  |
| property bool | [**isTransitioning**](classQf3DTerrainProvider.md#property-istransitioning-12)  <br>_Whether a transition to apply pan/zoom offsets has begun._  |
| property [**QgsQuickMapSettings**](classQgsQuickMapSettings.md) \* | [**mapSettings**](classQf3DTerrainProvider.md#property-mapsettings-12)  <br>_The map settings from which to get extent for terrain generation._  |
| property QVariantList | [**normalizedData**](classQf3DTerrainProvider.md#property-normalizeddata-12)  <br>_Normalized height data array [0.0-1.0] for terrain mesh generation._  |
| property QgsRectangle | [**normalizedDataExtent**](classQf3DTerrainProvider.md#property-normalizeddataextent-12)  <br>_Geographic extent used for terrain data._  |
| property double | [**offsetScale**](classQf3DTerrainProvider.md#property-offsetscale-12)  <br>_Offset scale from the last generated terrain data._  |
| property QVector3D | [**offsetVector**](classQf3DTerrainProvider.md#property-offsetvector-12)  <br>_Offset vector from the last generated terrain data._  |
| property QgsProject \* | [**project**](classQf3DTerrainProvider.md#property-project-12)  <br>_The project from which to read the terrain configuration and DEM layer._  |
| property QSizeF | [**size**](classQf3DTerrainProvider.md#property-size-12)  <br>_Size of the terrain._  |




## Public Signals

| Type | Name |
| ---: | :--- |
| signal void | [**extentChanged**](classQf3DTerrainProvider.md#signal-extentchanged)  <br> |
| signal void | [**forceSquareSizeChanged**](classQf3DTerrainProvider.md#signal-forcesquaresizechanged)  <br> |
| signal void | [**gridSizeChanged**](classQf3DTerrainProvider.md#signal-gridsizechanged)  <br> |
| signal void | [**isLoadingChanged**](classQf3DTerrainProvider.md#signal-isloadingchanged)  <br> |
| signal void | [**isTransitioningChanged**](classQf3DTerrainProvider.md#signal-istransitioningchanged)  <br> |
| signal void | [**mapSettingsChanged**](classQf3DTerrainProvider.md#signal-mapsettingschanged)  <br> |
| signal void | [**normalizedDataChanged**](classQf3DTerrainProvider.md#signal-normalizeddatachanged)  <br> |
| signal void | [**offsetScaleChanged**](classQf3DTerrainProvider.md#signal-offsetscalechanged)  <br> |
| signal void | [**offsetVectorChanged**](classQf3DTerrainProvider.md#signal-offsetvectorchanged)  <br> |
| signal void | [**projectChanged**](classQf3DTerrainProvider.md#signal-projectchanged)  <br> |
| signal void | [**terrainDataReady**](classQf3DTerrainProvider.md#signal-terraindataready)  <br>_Emitted when terrain data has been successfully loaded and is ready for use._  |




## Public Functions

| Type | Name |
| ---: | :--- |
|   | [**Qf3DTerrainProvider**](#function-qf3dterrainprovider) (QObject \* parent=nullptr) <br>_Creates a new terrain provider._  |
|  Q\_INVOKABLE void | [**beginTransition**](#function-begintransition) () <br>_Regenerates the extent and terrain data from current offset._  |
|  Q\_INVOKABLE void | [**endTransition**](#function-endtransition) () <br>_Applies offsets and reset values to zeros._  |
|  QgsRectangle | [**extent**](#function-extent-22) () const<br>_Returns the current extent taking into account offsets._  |
|  bool | [**forceSquareSize**](#function-forcesquaresize-22) () const<br> |
|  Q\_INVOKABLE QVector3D | [**geoTo3D**](#function-geoto3d-12) (double geoX, double geoY, float heightOffset=0.0f) const<br> |
|  Q\_INVOKABLE QVector3D | [**geoTo3D**](#function-geoto3d-22) (const QgsPoint & geoPoint, float heightOffset=0.0f) const<br> |
|  QSize | [**gridSize**](#function-gridsize-22) () const<br>_Returns the grid dimensions for terrain sampling._  |
|  Q\_INVOKABLE double | [**heightAt**](#function-heightat) (double x, double y) const<br> |
|  bool | [**isLoading**](#function-isloading-22) () const<br>_Returns TRUE if terrain data is currently being loaded._  |
|  bool | [**isTransitioning**](#function-istransitioning-22) () const<br> |
|  [**QgsQuickMapSettings**](classQgsQuickMapSettings.md) \* | [**mapSettings**](#function-mapsettings-22) () const<br>_Returns the map settings._  |
|  QVariantList | [**normalizedData**](#function-normalizeddata-22) () const<br>_Returns the normalized height data array [0.0-1.0]._  |
|  QgsRectangle | [**normalizedDataExtent**](#function-normalizeddataextent-22) () const<br>_Returns the geographic extent for the retrieved terrain data._  |
|  Q\_INVOKABLE double | [**normalizedHeightAt**](#function-normalizedheightat) (double x, double y) const<br> |
|  double | [**offsetScale**](#function-offsetscale-22) () const<br>_Returns the offset scale from the last generated terrain data._  |
|  QVector3D | [**offsetVector**](#function-offsetvector-22) () const<br>_Returns the offset scale from the last generated terrain data._  |
|  Q\_INVOKABLE void | [**pan**](#function-pan) (double x, double z) <br>_Applies X/Z offsets to the stored terrain based on pan action._  |
|  QgsProject \* | [**project**](#function-project-22) () const<br>_Returns the project from which to read terrain configuration and DEM layer._  |
|  Q\_INVOKABLE QgsPoint | [**scene3DToGeo**](#function-scene3dtogeo) (double sceneX, double sceneZ) const<br> |
|  void | [**setForceSquareSize**](#function-setforcesquaresize) (bool forceSquareSize) <br> |
|  void | [**setMapSettings**](#function-setmapsettings) ([**QgsQuickMapSettings**](classQgsQuickMapSettings.md) \* mapSettings) <br>_Sets the map settings._  |
|  void | [**setProject**](#function-setproject) (QgsProject \* project) <br>_Sets the project._  |
|  QSizeF | [**size**](#function-size-22) () const<br>_Returns the size of the terrain._  |
|  Q\_INVOKABLE void | [**zoom**](#function-zoom) (double factor) <br>_Applies scale offset to the stored terrain based on zoom action._  |
|   | [**~Qf3DTerrainProvider**](#function-qf3dterrainprovider) () override<br> |




























## Detailed Description


Provides terrain elevation data for 3D visualization from DEM raster layers or online elevation services.


This class handles elevation data retrieval from either a project's DEM raster layer or falls back to QGIS terrain providers (e.g., online Terrarium tiles) when no DEM is available. The data is normalized and provided as height samples for terrain mesh generation in Qt Quick 3D.




**Note:**

QML Type: [**Qf3DTerrainProvider**](classQf3DTerrainProvider.md) 





    
## Public Properties Documentation




### property extent [1/2]

_Geographic extent used for terrain data._ 
```C++
QgsRectangle Qf3DTerrainProvider::extent;
```




<hr>



### property forceSquareSize [1/2]

_Whether the extent's shorter side will be expanded to form a square terrain._ 
```C++
bool Qf3DTerrainProvider::forceSquareSize;
```




<hr>



### property gridSize [1/2]

_Grid dimensions (width x height) for terrain sampling._ 
```C++
QSize Qf3DTerrainProvider::gridSize;
```




<hr>



### property isLoading [1/2]

_Whether terrain data is currently being loaded._ 
```C++
bool Qf3DTerrainProvider::isLoading;
```




<hr>



### property isTransitioning [1/2]

_Whether a transition to apply pan/zoom offsets has begun._ 
```C++
bool Qf3DTerrainProvider::isTransitioning;
```




<hr>



### property mapSettings [1/2]

_The map settings from which to get extent for terrain generation._ 
```C++
QgsQuickMapSettings * Qf3DTerrainProvider::mapSettings;
```




<hr>



### property normalizedData [1/2]

_Normalized height data array [0.0-1.0] for terrain mesh generation._ 
```C++
QVariantList Qf3DTerrainProvider::normalizedData;
```




<hr>



### property normalizedDataExtent [1/2]

_Geographic extent used for terrain data._ 
```C++
QgsRectangle Qf3DTerrainProvider::normalizedDataExtent;
```




<hr>



### property offsetScale [1/2]

_Offset scale from the last generated terrain data._ 
```C++
double Qf3DTerrainProvider::offsetScale;
```




<hr>



### property offsetVector [1/2]

_Offset vector from the last generated terrain data._ 
```C++
QVector3D Qf3DTerrainProvider::offsetVector;
```




<hr>



### property project [1/2]

_The project from which to read the terrain configuration and DEM layer._ 
```C++
QgsProject * Qf3DTerrainProvider::project;
```




<hr>



### property size [1/2]

_Size of the terrain._ 
```C++
QSizeF Qf3DTerrainProvider::size;
```




<hr>
## Public Signals Documentation




### signal extentChanged 

```C++
void Qf3DTerrainProvider::extentChanged;
```




<hr>



### signal forceSquareSizeChanged 

```C++
void Qf3DTerrainProvider::forceSquareSizeChanged;
```




<hr>



### signal gridSizeChanged 

```C++
void Qf3DTerrainProvider::gridSizeChanged;
```




<hr>



### signal isLoadingChanged 

```C++
void Qf3DTerrainProvider::isLoadingChanged;
```




<hr>



### signal isTransitioningChanged 

```C++
void Qf3DTerrainProvider::isTransitioningChanged;
```




<hr>



### signal mapSettingsChanged 

```C++
void Qf3DTerrainProvider::mapSettingsChanged;
```




<hr>



### signal normalizedDataChanged 

```C++
void Qf3DTerrainProvider::normalizedDataChanged;
```




<hr>



### signal offsetScaleChanged 

```C++
void Qf3DTerrainProvider::offsetScaleChanged;
```




<hr>



### signal offsetVectorChanged 

```C++
void Qf3DTerrainProvider::offsetVectorChanged;
```




<hr>



### signal projectChanged 

```C++
void Qf3DTerrainProvider::projectChanged;
```




<hr>



### signal terrainDataReady 

_Emitted when terrain data has been successfully loaded and is ready for use._ 
```C++
void Qf3DTerrainProvider::terrainDataReady;
```




<hr>
## Public Functions Documentation




### function Qf3DTerrainProvider 

_Creates a new terrain provider._ 
```C++
explicit Qf3DTerrainProvider::Qf3DTerrainProvider (
    QObject * parent=nullptr
) 
```




<hr>



### function beginTransition 

_Regenerates the extent and terrain data from current offset._ 
```C++
Q_INVOKABLE void Qf3DTerrainProvider::beginTransition () 
```




<hr>



### function endTransition 

_Applies offsets and reset values to zeros._ 
```C++
Q_INVOKABLE void Qf3DTerrainProvider::endTransition () 
```




<hr>



### function extent [2/2]

_Returns the current extent taking into account offsets._ 
```C++
inline QgsRectangle Qf3DTerrainProvider::extent () const
```




<hr>



### function forceSquareSize [2/2]

```C++
bool Qf3DTerrainProvider::forceSquareSize () const
```




<hr>



### function geoTo3D [1/2]

```C++
Q_INVOKABLE QVector3D Qf3DTerrainProvider::geoTo3D (
    double geoX,
    double geoY,
    float heightOffset=0.0f
) const
```



Converts geographic coordinates to a 3D scene position. 

**Parameters:**


* `geoX` X coordinate in map CRS 
* `geoY` Y coordinate in map CRS 
* `heightOffset` additional vertical offset (default 0) 



**Returns:**

3D position in scene space, or a null vector if the extent is invalid 





        

<hr>



### function geoTo3D [2/2]

```C++
Q_INVOKABLE QVector3D Qf3DTerrainProvider::geoTo3D (
    const QgsPoint & geoPoint,
    float heightOffset=0.0f
) const
```



Converts geographic coordinates to a 3D scene position. 

**Parameters:**


* `geoPoint` Point coordinate in map CRS 



**Returns:**

3D position in scene space, or a null vector if the extent is invalid 





        

<hr>



### function gridSize [2/2]

_Returns the grid dimensions for terrain sampling._ 
```C++
inline QSize Qf3DTerrainProvider::gridSize () const
```




<hr>



### function heightAt 

```C++
Q_INVOKABLE double Qf3DTerrainProvider::heightAt (
    double x,
    double y
) const
```



Returns the real height value at the specified map coordinates. 

**Parameters:**


* `x` X coordinate in map CRS 
* `y` Y coordinate in map CRS 



**Returns:**

Height in map units, or 0 if outside extent 





        

<hr>



### function isLoading [2/2]

_Returns TRUE if terrain data is currently being loaded._ 
```C++
bool Qf3DTerrainProvider::isLoading () const
```




<hr>



### function isTransitioning [2/2]

```C++
inline bool Qf3DTerrainProvider::isTransitioning () const
```




<hr>



### function mapSettings [2/2]

_Returns the map settings._ 
```C++
QgsQuickMapSettings * Qf3DTerrainProvider::mapSettings () const
```




<hr>



### function normalizedData [2/2]

_Returns the normalized height data array [0.0-1.0]._ 
```C++
QVariantList Qf3DTerrainProvider::normalizedData () const
```




<hr>



### function normalizedDataExtent [2/2]

_Returns the geographic extent for the retrieved terrain data._ 
```C++
inline QgsRectangle Qf3DTerrainProvider::normalizedDataExtent () const
```




<hr>



### function normalizedHeightAt 

```C++
Q_INVOKABLE double Qf3DTerrainProvider::normalizedHeightAt (
    double x,
    double y
) const
```



Returns the normalized height value [0.0-1.0] at the specified map coordinates. 

**Parameters:**


* `x` X coordinate in map CRS 
* `y` Y coordinate in map CRS 



**Returns:**

Normalized height between 0 and 1, or 0 if outside extent 





        

<hr>



### function offsetScale [2/2]

_Returns the offset scale from the last generated terrain data._ 
```C++
inline double Qf3DTerrainProvider::offsetScale () const
```




<hr>



### function offsetVector [2/2]

_Returns the offset scale from the last generated terrain data._ 
```C++
inline QVector3D Qf3DTerrainProvider::offsetVector () const
```




<hr>



### function pan 

_Applies X/Z offsets to the stored terrain based on pan action._ 
```C++
Q_INVOKABLE void Qf3DTerrainProvider::pan (
    double x,
    double z
) 
```




<hr>



### function project [2/2]

_Returns the project from which to read terrain configuration and DEM layer._ 
```C++
QgsProject * Qf3DTerrainProvider::project () const
```




<hr>



### function scene3DToGeo 

```C++
Q_INVOKABLE QgsPoint Qf3DTerrainProvider::scene3DToGeo (
    double sceneX,
    double sceneZ
) const
```



Converts a 3D scene position back to geographic coordinates in map CRS. Inverse of [**geoTo3D()**](classQf3DTerrainProvider.md#function-geoto3d-12); returns an empty QgsPoint when the extent is invalid. 


        

<hr>



### function setForceSquareSize 

```C++
void Qf3DTerrainProvider::setForceSquareSize (
    bool forceSquareSize
) 
```




<hr>



### function setMapSettings 

_Sets the map settings._ 
```C++
void Qf3DTerrainProvider::setMapSettings (
    QgsQuickMapSettings * mapSettings
) 
```




<hr>



### function setProject 

_Sets the project._ 
```C++
void Qf3DTerrainProvider::setProject (
    QgsProject * project
) 
```




<hr>



### function size [2/2]

_Returns the size of the terrain._ 
```C++
inline QSizeF Qf3DTerrainProvider::size () const
```




<hr>



### function zoom 

_Applies scale offset to the stored terrain based on zoom action._ 
```C++
Q_INVOKABLE void Qf3DTerrainProvider::zoom (
    double factor
) 
```




<hr>



### function ~Qf3DTerrainProvider 

```C++
Qf3DTerrainProvider::~Qf3DTerrainProvider () override
```




<hr>

------------------------------
The documentation for this class was generated from the following file `src/3d/qf3dterrainprovider.h`

