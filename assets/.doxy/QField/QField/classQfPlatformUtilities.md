...
|  Q\_INVOKABLE bool | [**renameFileWithSidecars**](#function-renamefilewithsidecars) (const QString & oldFilePath, const QString & newFilePath, bool overwrite=true) const<br> |
...
| virtual Q\_INVOKABLE void | [**sendCompressedFolderTo**](#function-sendcompressedfolderto) (const QString & path, const QString & fileName=QString()) const<br> |
...
### function renameFileWithSidecars 

```C++
Q_INVOKABLE bool QfPlatformUtilities::renameFileWithSidecars (
    const QString & oldFilePath,
    const QString & newFilePath,
    bool overwrite=true
) const
...
Renames _oldFilePath_ to _newFilePath_ along with the sidecar files accompanying it, each taking the new file's name while keeping its own suffix.


Returns the outcome of renaming the file itself; a sidecar which could not follow is logged and does not turn a successful move into a failure, as the file has moved either way and the caller's path is valid. 
...
Compresses a folder _path_ and sends it via the platform native API. When _fileName_ is set, it is used as the resulting archive name (without extension); otherwise the folder name is used. 
