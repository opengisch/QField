...
[**ClassList**](annotated.md) **>** [**QfCloudUtils**](classQfCloudUtils.md)
...
| Type | Name |
| ---: | :--- |
| property QString | [**errorCodeOverQuota**](classQfCloudUtils.md#property-errorcodeoverquota-12)  <br> |
| property QString | [**errorCodePlanInsufficient**](classQfCloudUtils.md#property-errorcodeplaninsufficient-12)  <br> |
...
| Type | Name |
| ---: | :--- |
|  Q\_INVOKABLE void | [**addPendingAttachments**](#function-addpendingattachments) (const QString & username, const QString & projectId, const QStringList & fileNames, [**QfCloudConnection**](classQfCloudConnection.md) \* cloudConnection=nullptr, const bool & checkSumCheck=false) <br> |
|  Q\_INVOKABLE QString | [**checkCloudifyFeasibility**](#function-checkcloudifyfeasibility) (const QString & localPath) <br> |
...
|  QList&lt; [**QfCloudDelta**](structQfCloudDelta.md) &gt; | [**parseDeltaJsonDocument**](#function-parsedeltajsondocument) (const QJsonDocument & jsonDocument, QString & errorString, bool & isValid) <br> |
...
|  Q\_INVOKABLE QString | [**subscriptionManagementUrl**](#function-subscriptionmanagementurl) (const QString & serverUrl, const QString & plan, const QString & projectOwner, const QString & username) <br> |
|  QString | [**summarizeDeltaContent**](#function-summarizedeltacontent) (const QList&lt; QJsonObject &gt; & deltaObjects, const QString & modificationSeparator=QStringLiteral(", "), const QString & layerSeparator=QStringLiteral(" / ")) <br> |
...
### property errorCodeOverQuota [1/2]

```C++
QString QfCloudUtils::errorCodeOverQuota;
```
...
### property errorCodePlanInsufficient [1/2]

```C++
QString QfCloudUtils::errorCodePlanInsufficient;
```
...
```C++
static Q_INVOKABLE void QfCloudUtils::addPendingAttachments (
    const QString & username,
    const QString & projectId,
    const QStringList & fileNames,
    QfCloudConnection * cloudConnection=nullptr,
...
### function checkCloudifyFeasibility 

```C++
static Q_INVOKABLE QString QfCloudUtils::checkCloudifyFeasibility (
...
Checks whether the project found in _localPath_ can be stored on QFieldCloud, and returns the reason why it cannot, or an empty string when it can. _localPath_ can either be a project file or the folder holding it. 
...
```C++
static Q_INVOKABLE QString QfCloudUtils::documentationFromErrorString (
...
```C++
static const QMultiMap< QString, QString > QfCloudUtils::getPendingAttachments (
    const QString & username
) 
```
...
```C++
static Q_INVOKABLE const QString QfCloudUtils::getProjectId (
...
```C++
static Q_INVOKABLE bool QfCloudUtils::hasPendingAttachments (
    const QString & username
) 
```
...
```C++
static bool QfCloudUtils::isCloudAction (
...
```C++
static const QString QfCloudUtils::localCloudDirectory () 
```
...
```C++
static const QString QfCloudUtils::localProjectFilePath (
    const QString & username,
    const QString & projectId
) 
```
...
### function parseDeltaJsonDocument 

```C++
static QList< QfCloudDelta > QfCloudUtils::parseDeltaJsonDocument (
...
```C++
static const QVariant QfCloudUtils::projectSetting (
    const QString & projectId,
...
```C++
static Q_INVOKABLE void QfCloudUtils::removePendingAttachment (
    const QString & username,
    const QString & projectId,
    const QString & fileName
) 
...
```C++
static void QfCloudUtils::setLocalCloudDirectory (
    const QString & path
) 
```
...
```C++
static void QfCloudUtils::setProjectSetting (
    const QString & projectId,
    const QString & setting,
...
```C++
static Q_INVOKABLE QString QfCloudUtils::subscriptionManagementUrl (
    const QString & serverUrl,
    const QString & plan,
    const QString & projectOwner,
    const QString & username
) 
```
...
Returns the URL for managing the subscription for a given _plan_, _serverUrl_, and _username_. Returns an empty string if the server is not the default QFieldCloud server or if the _projectOwner_ does not match the _username_. 
...
### function summarizeDeltaContent 

```C++
static QString QfCloudUtils::summarizeDeltaContent (
    const QList< QJsonObject > & deltaObjects,
    const QString & modificationSeparator=QStringLiteral(", "),
    const QString & layerSeparator=QStringLiteral(" / ")
) 
```
...
```C++
static Q_INVOKABLE QString QfCloudUtils::userFriendlyErrorString (
    const QString & errorString
) 
```
