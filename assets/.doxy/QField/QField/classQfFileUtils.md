...
|  QSet&lt; QString &gt; | [**sidecarFilesForPath**](#function-sidecarfilesforpath) (const QString & filePath) <br> |
...
### function sidecarFilesForPath 

```C++
static QSet< QString > QfFileUtils::sidecarFilesForPath (
    const QString & filePath
) 
```
...
Returns the existing files accompanying _filePath_ which carry no meaning on their own and must therefore follow it wherever it goes - when shared, exported, moved into an attachment folder or deleted.


This extends QgsFileUtils::sidecarFilesForPath(), which resolves companion files by asking the registered data providers and so covers datasets only, with additional files that are relevant to QField. 
