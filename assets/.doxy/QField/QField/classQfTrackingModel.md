...
[**ClassList**](annotated.md) **>** [**QfTrackingModel**](classQfTrackingModel.md)
...
| Type | Name |
| ---: | :--- |
| property int | [**activeCount**](classQfTrackingModel.md#property-activecount-12)  <br> |
...
| Type | Name |
| ---: | :--- |
| signal void | [**activeCountChanged**](classQfTrackingModel.md#signal-activecountchanged)  <br> |
| signal void | [**layerInTrackingChanged**](classQfTrackingModel.md#signal-layerintrackingchanged) (QgsVectorLayer \* layer, bool tracking) <br> |
| signal void | [**trackingSetupRequested**](classQfTrackingModel.md#signal-trackingsetuprequested) (QModelIndex trackerIndex, bool skipSettings) <br> |
...
| Type | Name |
| ---: | :--- |
|   | [**QfTrackingModel**](#function-qftrackingmodel) (QObject \* parent=nullptr) <br> |
|  int | [**activeCount**](#function-activecount-22) () const<br>_Returns the active tracker count._  |
...
|  Q\_INVOKABLE void | [**startTracker**](#function-starttracker) (QgsVectorLayer \* layer, const [**QfGnssPositionInformation**](classQfGnssPositionInformation.md) & positionInformation=[**QfGnssPositionInformation**](classQfGnssPositionInformation.md)(), const QgsPoint & projectedPosition=QgsPoint()) <br>_Starts tracking for the provided vector_ _layer_ _provided it has a tracking session created._ |
...
|  Q\_INVOKABLE [**QfTracker**](classQfTracker.md) \* | [**trackerForLayer**](#function-trackerforlayer) (QgsVectorLayer \* layer) const<br>_Returns the tracker for the vector_ _layer_ _if a tracking session is present, otherwise returns NULLPTR._ |
|  Q\_INVOKABLE void | [**trackingSetupDone**](#function-trackingsetupdone) () <br> |
|   | [**~QfTrackingModel**](#function-qftrackingmodel) () override<br> |
...
```C++
enum QfTrackingModel::TrackingRoles {
...
### property activeCount [1/2]

```C++
int QfTrackingModel::activeCount;
```
...
```C++
void QfTrackingModel::activeCountChanged;
```
...
```C++
void QfTrackingModel::layerInTrackingChanged;
```
...
```C++
void QfTrackingModel::trackingSetupRequested;
```
...
### function QfTrackingModel 

```C++
explicit QfTrackingModel::QfTrackingModel (
...
### function activeCount [2/2]

_Returns the active tracker count._ 
```C++
...
```C++
Q_INVOKABLE QList< QgsVectorLayer * > QfTrackingModel::availableLayers (
    QgsProject * project
) const
```
...
```C++
Q_INVOKABLE QgsVectorLayer * QfTrackingModel::bestAvailableLayer (
    QgsProject * project
) const
```
...
```C++
int QfTrackingModel::columnCount (
...
```C++
Q_INVOKABLE void QfTrackingModel::createProjectTrackers (
    QgsProject * project
) 
```
...
```C++
Q_INVOKABLE QModelIndex QfTrackingModel::createTracker (
...
```C++
Q_INVOKABLE bool QfTrackingModel::featureInTracking (
    QgsVectorLayer * layer,
...
```C++
Q_INVOKABLE bool QfTrackingModel::featuresInTracking (
    QgsVectorLayer * layer,
...
```C++
QModelIndex QfTrackingModel::index (
    int row,
    int column,
    const QModelIndex & parent=QModelIndex()
) override const
...
```C++
Q_INVOKABLE bool QfTrackingModel::layerInActiveTracking (
    QgsVectorLayer * layer
) const
```
...
```C++
Q_INVOKABLE bool QfTrackingModel::layerInTracking (
    QgsVectorLayer * layer
) const
```
...
```C++
QModelIndex QfTrackingModel::parent (
    const QModelIndex & index
) override const
```
...
```C++
Q_INVOKABLE void QfTrackingModel::replayPositionInformationList (
...
```C++
Q_INVOKABLE void QfTrackingModel::requestTrackingSetup (
    QgsVectorLayer * layer,
...
```C++
QHash< int, QByteArray > QfTrackingModel::roleNames () override const
```
...
```C++
virtual bool QfTrackingModel::setData (
    const QModelIndex & index,
...
```C++
Q_INVOKABLE void QfTrackingModel::setTrackerVisibility (
    QgsVectorLayer * layer,
    bool visible
) 
```
...
```C++
Q_INVOKABLE void QfTrackingModel::startTracker (
    QgsVectorLayer * layer,
...
```C++
Q_INVOKABLE void QfTrackingModel::stopTracker (
    QgsVectorLayer * layer
) 
```
...
```C++
Q_INVOKABLE void QfTrackingModel::stopTrackers () 
```
...
```C++
Q_INVOKABLE void QfTrackingModel::suspendUntilReplay () 
```
...
```C++
Q_INVOKABLE QfTracker * QfTrackingModel::trackerForLayer (
    QgsVectorLayer * layer
) const
```
...
```C++
Q_INVOKABLE void QfTrackingModel::trackingSetupDone () 
```
...
```C++
QfTrackingModel::~QfTrackingModel () override
```
...
------------------------------
The documentation for this class was generated from the following file `src/core/qftrackingmodel.h`
