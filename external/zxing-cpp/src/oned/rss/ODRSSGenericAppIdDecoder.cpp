...
*/
  struct DecodedChar : public DecodedValue
  {
      ...
      DecodedChar() = default;
      DecodedChar( int np, char c )
        : DecodedValue( np ), value( c ) {}
...
*/
  struct DecodedInformation : public DecodedValue
  {
      ...
      DecodedInformation( int np, std::string s )
        : DecodedValue( np ), newString( std::move( s ) ) {}
      DecodedInformation( int np, std::string s, int r )
        : DecodedValue( np ), newString( std::move( s ) ), remainingValue( r ) {}
...
*/
  struct DecodedNumeric : public DecodedValue
  {
      ...
      DecodedNumeric( int newPosition, int first, int second )
        : DecodedValue( newPosition ), firstDigit( first ), secondDigit( second )
      {
    ...
    ParseBlocks( const BitArray &bits, ParsingState &state, std::string &buffer )
  {
    while ( true )
    {
      ...
      auto result = state.encoding == ParsingState::ALPHA ? ParseAlphaBlock( bits, state, buffer ) : ( state.encoding == ParsingState::ISO_IEC_646 ? ParseIsoIec646Block( bits, state, buffer ) :
                                                                                                                                                   // else
