...
namespace ZXing::OneD
{
  ...
  static bool IsCharacterPair( const PatternView &v )
  {
    ...
    return IsCharacter( LeftChar( v ), 17, modSizeRef ) && ( v.size() == HALF_PAIR_SIZE || IsCharacter( RightChar( v ), 17, modSizeRef ) );
  }
  ...
  {
    auto checksum = TransformReduce( pairs, 0, []( auto p ) { return p.left.checksum + p.right.checksum; } ) % 211 + 211 * ( 2 * Size( pairs ) - 4 - !pairs.back().right );
  ...
  static Pairs ReadRowOfPairs( PatternView &next, int rowNumber )
  {
    ...
    if constexpr ( STACKED )
    {
      ...
      while ( next.shift( 1 ) )
      {
        if ( IsL2RPair( next ) && ( pair = ReadPair( next, Direction::Right ) ) && ( pair.finder != FINDER_A || IsGuard( next[-1], next[11] ) ) )
          break;
    ...
    else
    {
      ...
      while ( next.shift( 2 ) )
      {
        if ( IsL2RPair( next ) && IsGuard( next[-1], next[11] ) && ( pair = ReadPair( next, Direction::Right ) ).finder == FINDER_A )
          break;
      }
                                               ...
                                               std::unique_ptr<RowReader::DecodingState> &state ) const
  {
    ...
    // the first
    return { TextDecoder::FromLatin1( txt ), EstimatePosition( pairs.front(), pairs.back() ), BarcodeFormat::DataBarExpanded, std::move( symbologyIdentifier ), {}, {}, false, EstimateLineCount( pairs.front(), pairs.back() ) };
  }
