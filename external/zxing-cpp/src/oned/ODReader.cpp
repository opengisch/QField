...
namespace ZXing::OneD
{
  ...
  Reader::Reader( const DecodeHints &hints )
    : _tryHarder( hints.tryHarder() ), _tryRotate( hints.tryRotate() ), _isPure( hints.isPure() ), _minLineCount( hints.minLineCount() )
  {
    _readers.reserve( 8 );
            ...
            if ( result.isValid() )
            {
              ...
              for ( auto &other : res )
              {
                if ( other == result )
                {
                  ...
                  if ( dTop < dBot || ( dTop == dBot && rotate ^ ( sumAbsComponent( points[0] ) > sumAbsComponent( result.position()[0] ) ) ) )
                  {
