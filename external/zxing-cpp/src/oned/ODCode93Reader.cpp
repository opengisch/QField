...
*/
  ...
  static bool IsStartGuard( const PatternView &window, int spaceInPixel )
  {
    ...
    return IsPattern( window, FixedPattern<4, 4> { 1, 1, 1, 1 }, spaceInPixel, QUIET_ZONE_SCALE * 12 ) && window[4] > 3 * window[5] - 2 && RowReader::OneToFourBitPattern<CHAR_LEN, CHAR_SUM>( window ) == ASTERISK_ENCODING;
  }
