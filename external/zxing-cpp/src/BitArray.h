...
*/
  class BitArray
  {
    ...
    public:
      ...
      class Iterator
      {
        public:
          ...
          {
            return static_cast<int>( _value - rhs._value ) * 32 + ( _mask >= rhs._mask ? +BitHacks::CountBitsSet( _mask - rhs._mask ) : -BitHacks::CountBitsSet( rhs._mask - _mask ) );
          }
        ...
        private:
          Iterator( std::vector<uint32_t>::const_iterator p, uint32_t m )
            : _value( p ), _mask( m ) {}
	...
	*/
      ...
      Iterator iterAt( int i ) const noexcept
      {
        return { _bits.cbegin() + ( i >> 5 ), 1U << ( i & 0x1F ) };
      }
