...
namespace ZXing
{
  ...
  HybridBinarizer::HybridBinarizer( const ImageView &iv )
    : GlobalHistogramBinarizer( iv ) {}
      ...
      for ( int x = 0; x < subWidth; x++ )
      {
        ...
        if ( max - min <= MIN_DYNAMIC_RANGE )
        {
          ...
          if ( y > 0 && x > 0 )
          {
            ...
            int averageNeighborBlackPoint = ( blackPoints( x, y - 1 ) + ( 2 * blackPoints( x - 1, y ) ) + blackPoints( x - 1, y - 1 ) ) / 4;
...
*/
  ...
  std::shared_ptr<const BitMatrix> HybridBinarizer::getBlackMatrix() const
  {
    if ( width() >= MINIMUM_DIMENSION && height() >= MINIMUM_DIMENSION )
    {
      ...
      auto blackPoints = CalculateBlackPoints( luminances, subWidth, subHeight, width(), height(), _buffer.rowStride() );
