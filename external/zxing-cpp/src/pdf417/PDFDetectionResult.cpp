...
namespace ZXing
{
  namespace Pdf417
  {
    ...
    DetectionResult::DetectionResult( const BarcodeMetadata &barcodeMetadata, const Nullable<BoundingBox> &boundingBox )
      : _barcodeMetadata( barcodeMetadata ), _detectionResultColumns( barcodeMetadata.columnCount() + 2 ), _boundingBox( boundingBox )
    {
    }
    ...
    static void AdjustRowNumbersFromBothRI( std::vector<Nullable<DetectionResultColumn>> &detectionResultColumns )
    {
      ...
      {
        if ( LRIcodewords[codewordsRow] != nullptr && RRIcodewords[codewordsRow] != nullptr && LRIcodewords[codewordsRow].value().rowNumber() == RRIcodewords[codewordsRow].value().rowNumber() )
        {
