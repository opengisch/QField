    ...
    while ( i < length )
    {
      ...
      switch ( subMode )
      {
        ...
        case Mode::LOWER:
          ...
          else if ( subModeCh == 27 && subMode == Mode::ALPHA )
          { // LL
            subMode = Mode::LOWER;
          }
        ...
        case Mode::PUNCT:
          ...
          { // 29 AL - note not ignored if followed by Shift to Byte, 5.4.2.4 (b) (2)
            subMode = Mode::ALPHA;
          }
          break;
...
*/
                                 ...
                                 int &next )
  {
    ...
    fileId.fill( '0' );
    for ( ; codeIndex < codewords[0] && codewords[codeIndex] != MACRO_PDF417_TERMINATOR && codewords[codeIndex] != BEGIN_MACRO_PDF417_OPTIONAL_FIELD;
          codeIndex++ )
    {
