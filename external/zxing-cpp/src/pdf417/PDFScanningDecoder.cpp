    ...
    static int GetStartColumn( const DetectionResult &detectionResult, int barcodeColumn, int imageRow, bool leftToRight )
    {
      ...
      while ( IsValidBarcodeColumn( detectionResult, barcodeColumn - offset ) )
      {
        ...
        for ( auto &previousRowCodeword : detectionResult.column( barcodeColumn ).value().allCodewords() )
        {
          ...
          {
            return ( leftToRight ? previousRowCodeword.value().endX() : previousRowCodeword.value().startX() ) + offset * skippedColumns * ( previousRowCodeword.value().endX() - previousRowCodeword.value().startX() );
          }
        }
        skippedColumns++;
      }
...
*/
    ...
    {
      if ( Size( erasures ) > numECCodewords / 2 + MAX_ERRORS || numECCodewords < 0 || numECCodewords > MAX_EC_CODEWORDS )
      {
