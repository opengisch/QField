  ...
  namespace DecodedBitStreamParser
  {
    ...
    const static std::array<short, 0x40> CHARSETS[] = {
      ...
      },
      {
        ...
        GS,
        RS,
        NS, // Note that in original code in Java, NS is not there, which seems to be a bug
        0xDB,
        0xDC,
        0xDD,
        0xDE,
	...
	*/
    static void ParseStructuredAppend( const ByteArray &bytes, int &i, StructuredAppendInfo &sai )
    {
      ...
      if ( sai.count == 1 || sai.count <= sai.index )
    ...
    DecoderResult Decode( ByteArray &&bytes, const int mode, const std::string &characterSet )
    {
      ...
      switch ( mode )
      {
        ...
        case 3:
        {
          ...
          if ( result.size() >= 9 && result.compare( 0, 7, L"[)>\u001E01\u001D" ) == 0 )
          { // "[)>" + RS + "01" + GS
