...
*/
  ...
  {
    if ( !IsValidPoint( bullsEyeCorners[0], image.width(), image.height() ) || !IsValidPoint( bullsEyeCorners[1], image.width(), image.height() ) || !IsValidPoint( bullsEyeCorners[2], image.width(), image.height() ) || !IsValidPoint( bullsEyeCorners[3], image.width(), image.height() ) )
    {
      return false;
    }
    ...
    if ( compact )
    {
      ...
      if ( nbLayers == 1 && ( correctedData & 0x20 ) )
      { // ISO/IEC 24778:2008 Section 9 MSB artificially set
        readerInit = true;
        correctedData &= ~0x20;
      }
    ...
    else
    {
      ...
      if ( nbLayers <= 22 && ( correctedData & 0x400 ) )
      { // ISO/IEC 24778:2008 Section 9 MSB artificially set
        readerInit = true;
        correctedData &= ~0x400;
      }
...
*/
  ...
  {
    if ( !IsValidPoint( p1.x, p1.y, image.width(), image.height() ) || !IsValidPoint( p2.x, p2.y, image.width(), image.height() ) )
      return 0;
