  ...
  AztecData GetEncodedData( const BitArray &bits, const std::string &characterSet )
  {
    ...
    while ( remBits )
    {
      ...
      else
      {
        ...
        else if ( std::strcmp( str, "FLGN" ) == 0 )
        {
          ...
          {
            // FLG(1) to FLG(6) ECI
            encoding = CharacterSetECI::OnChangeAppendReset( ParseECIValue( remBits, flg ), res.text, text, encoding );
          }
          else
          {
            // FLG(7) is invalid
    ...
    if ( !res.text.empty() )
    {
      ...
      if ( haveFNC1 )
      {
        ...
        else if ( res.text.size() > 3 && std::isdigit( res.text[0] ) && std::isdigit( res.text[1] ) && res.text[2] == 29 )
        {
