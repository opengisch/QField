...
TEST_CASE( "BookmarkModel" )
{
  ...
  SECTION( "rowCount reflects bookmarks from both user and project managers" )
  {
    ...
    QfBookmarkModel model( &userManager, &projectManager );
    REQUIRE( model.rowCount( QModelIndex() ) == 2 );
  }
  ...
  SECTION( "BookmarkName role returns correct name" )
  {
    ...
    REQUIRE( model.index( 0, 0 ).data( QfBookmarkModel::BookmarkName ).toString() == QStringLiteral( "MyBookmark" ) );
  }
  ...
  SECTION( "BookmarkGroup role returns correct group" )
  {
    ...
    QfBookmarkModel model( &manager, QgsApplication::bookmarkManager() );
    REQUIRE( model.index( 0, 0 ).data( QfBookmarkModel::BookmarkGroup ).toString() == QStringLiteral( "Favourites" ) );
  }
  ...
  SECTION( "BookmarkId role returns non-empty id" )
  {
    ...
    QfBookmarkModel model( &manager, QgsApplication::bookmarkManager() );
    REQUIRE_FALSE( model.index( 0, 0 ).data( QfBookmarkModel::BookmarkId ).toString().isEmpty() );
  }
  ...
  SECTION( "BookmarkPoint role returns center of extent" )
  {
    ...
    QfBookmarkModel model( &manager, QgsApplication::bookmarkManager() );
    const QgsGeometry point = model.index( 0, 0 ).data( QfBookmarkModel::BookmarkPoint ).value<QgsGeometry>();
  ...
  SECTION( "BookmarkCrs role returns the extent's CRS" )
  {
    ...
    manager.addBookmark( bm );
    QfBookmarkModel model( &manager, QgsApplication::bookmarkManager() );
    const QgsCoordinateReferenceSystem crs = model.index( 0, 0 ).data( QfBookmarkModel::BookmarkCrs ).value<QgsCoordinateReferenceSystem>();
  ...
  SECTION( "BookmarkUser is true for bookmarks in user manager, false for project manager" )
  {
    ...
    for ( int i = 0; i < model.rowCount( QModelIndex() ); ++i )
    {
      ...
      const QString name = idx.data( QfBookmarkModel::BookmarkName ).toString();
      const bool isUser = idx.data( QfBookmarkModel::BookmarkUser ).toBool();
  ...
  SECTION( "data returns invalid QVariant for invalid index" )
  {
    QgsBookmarkManager manager;
    QfBookmarkModel model( &manager, QgsApplication::bookmarkManager() );
    REQUIRE_FALSE( model.data( QModelIndex(), QfBookmarkModel::BookmarkName ).isValid() );
  }
  ...
  SECTION( "roleNames contains all custom roles" )
  {
    ...
    REQUIRE( roles.contains( QfBookmarkModel::BookmarkId ) );
    REQUIRE( roles.contains( QfBookmarkModel::BookmarkName ) );
    REQUIRE( roles.contains( QfBookmarkModel::BookmarkGroup ) );
    REQUIRE( roles.contains( QfBookmarkModel::BookmarkPoint ) );
    REQUIRE( roles.contains( QfBookmarkModel::BookmarkCrs ) );
    REQUIRE( roles.contains( QfBookmarkModel::BookmarkUser ) );
  }
  ...
  SECTION( "removeBookmark reduces rowCount" )
  {
    ...
    REQUIRE( model.rowCount( QModelIndex() ) == 1 );
    REQUIRE( model.index( 0, 0 ).data( QfBookmarkModel::BookmarkName ).toString() == QStringLiteral( "ToKeep" ) );
  }
  ...
  SECTION( "updateBookmarkDetails changes name and group" )
  {
    ...
    REQUIRE( model.index( 0, 0 ).data( QfBookmarkModel::BookmarkName ).toString() == QStringLiteral( "NewName" ) );
    REQUIRE( model.index( 0, 0 ).data( QfBookmarkModel::BookmarkGroup ).toString() == QStringLiteral( "NewGroup" ) );
  }
  ...
  SECTION( "setMapSettings emits mapSettingsChanged only on actual change" )
  {
    QgsBookmarkManager manager;
    QfBookmarkModel model( &manager, QgsApplication::bookmarkManager() );
    QSignalSpy spy( &model, &QfBookmarkModel::mapSettingsChanged );
