...
if [[ ${TRAVIS_SECURE_ENV_VARS} = true ]]; then
  ...
  elif [[ -n ${TRAVIS_TAG} ]] && [[ ${TRAVIS_BRANCH} =~ ^release-1_[0-9]+$  ]]; then
  ...
  elif [[ ${TRAVIS_BRANCH} = master ]] || [[ ${TRAVIS_BRANCH} =~ ^release-1_[0-9]+$ ]]; then
    # we are on a standard commit (i.e. no tag) on master or release-1* branch
    # write comment
