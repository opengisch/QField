...
CI_PULL_REQUEST_NUMBER=${CI_PULL_REQUEST_NUMBER:=$(jq --raw-output .pull_request.number "$GITHUB_EVENT_PATH")}
CI_PULL_REQUEST=$(( CI_PULL_REQUEST_NUMBER > 0 ))
...
export CI_PULL_REQUEST_NUMBER=${CI_PULL_REQUEST_NUMBER:=false}
...
export CI_UPLOAD_ARTIFACT_ID=${CI_UPLOAD_ARTIFACT_ID:=$( [[ ${CI_PULL_REQUEST} =~ false ]] && echo ${CI_TAG} || echo ${CI_PULL_REQUEST_NUMBER} )}
